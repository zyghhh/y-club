...
public class PracticeSetServiceImpl implements PracticeSetService {
    ...
    @Override
    public Map<String,Object> assemblePracticeSet(PracticeSubjectDTO dto) {
        ...
        practiceSetPO.setSetHeat(0);
        practiceSetPO.setCreatedBy(LoginUtil.getLoginId());
        practiceSetPO.setCreatedTime(new Date());
        params.put("practiceSetPO",practiceSetPO);
        params.put("practiceList",practiceList);
        return params;
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public PracticeSetVO insertPracticeSet(Map<String,Object> params) throws Exception {
        PracticeSetVO setVO = new PracticeSetVO();
        PracticeSetPO practiceSetPO = (PracticeSetPO)params.get("practiceSetPO");
        List<PracticeSubjectDetailVO> practiceList = (List<PracticeSubjectDetailVO>)params.get("practiceList");
        if(Objects.isNull(practiceSetPO) || CollectionUtils.isEmpty(practiceList)){
            throw new Exception("组装套卷失败");
        }
        practiceSetDao.add(practiceSetPO);
        Long practiceSetId = practiceSetPO.getId();
        List<PracticeSetDetailPO> practiceSetDetailPOList = new LinkedList<>();
        practiceList.forEach(e -> {
            ...
            practiceSetDetailPOList.add(detailPO);
        });
        practiceSetDetailDao.batchInsert(practiceSetDetailPOList);
    ...
    private List<PracticeSubjectDetailVO> getPracticeList(PracticeSubjectDTO dto) {
        ...
        Integer briefSubjectCount = 5;
        Integer totalSubjectCount = 25;
        //查询单选
        ...
        //查询简答
        dto.setSubjectCount(multipleSubjectCount);
        dto.setSubjectType(SubjectInfoTypeEnum.BRIEF.getCode());
        assembleList(dto,practiceSubjectListVOS,excludeSubjectIds);
